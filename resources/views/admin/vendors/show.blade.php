...
<div class="card position-relative" style="height: 500px;">
  ...
  <div class="card-body body-profile">
    ...
    <div class="mt-5">
      ...
      <div class="badge bg-warning">{{ __("Vendor is review by admin") }}</div>
      @endif
      @if ($vendor->vendor_status_id == 3)
      <div class="badge bg-success">{{ __("Vendor is approved") }}</div>
      @endif
      @if ($vendor->vendor_status_id == 4)
      <div class="badge bg-danger">{{ __("Vendor is declined") }}</div>
      @endif
  ...
  <div class="card-body">
    ...
    <div class="tab-content">
      <div class="tab-pane container active" id="aboutTab">
        ...
        @if ($vendor->vendor_status_id == 2)
          <div class="mb-5">
            <form class="d-inline" action="{{ route('app.vendors.approve', $vendor->id) }}" method="POST" id="formApproveVendor"
              with-submit-crud>
              @csrf
              @method("PUT")

              <button class="btn btn-success btn-sm" type="button"
                onclick="CORE.promptForm('formApproveVendor', '{{ __('This vendor will be approved!') }}')">{{ __("Approve This
                Vendor") }}</button>
            </form>
            <form class="d-inline" action="{{ route('app.vendors.decline', $vendor->id) }}" method="POST" id="formDeclineVendor"
              with-submit-crud>
              @csrf
              @method("PUT")

              <button class="btn btn-danger btn-sm" type="button"
                onclick="CORE.promptForm('formDeclineVendor', '{{ __('This vendor will be declined!') }}')">{{ __("Declined This
                Vendor") }}</button>
            </form>
          </div>
        @endif
