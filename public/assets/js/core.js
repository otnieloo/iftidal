const CORE = {
      ...
      inputRegion.addEventListener(
        ...
        CORE.delay(function () {
          if (inputRegion.value != "") {
            fetch(`${CORE.baseUrl}/app/regions/search?q=${inputRegion.value}`, {
              method: "POST",
              headers: {
              ...
              .then((response) => {
                const template = document.querySelector(`#listRegionSearch`);
                let html = "";
  ...
  dataTableServer: function (
    ...
    let text = emptyText != false ? emptyText : "No data available in table";
    if (dataObject == false) {
      dataObject = {
  ...
  onerrorProfileVendor(target) {
    target.onerror = null;
    target.src = `${CORE.baseUrl}/assets/images/img-user.png`;
  },
